    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-5 2xl:grid-cols-[1.08fr_0.92fr]">
                  ...
                  <div key={goal.id} className={`family-profile-goal-card ${goal.status === "done" ? "family-profile-goal-card-done" : ""}`}>
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      ...
                      {goal.status === "active" && canEdit ? (
                        <button type="button" onClick={() => onCompleteGoal(goal.id)} className="family-btn family-btn-primary">
                          Mark finished
                        </button>
                      ) : null}
                      {goal.status === "done" && canEdit && !goal.sharedAt ? (
                        <button type="button" onClick={() => onShareGoal(goal.id)} className="family-btn family-btn-secondary">
                          Share with family
                        </button>
                      ) : null}
                  ...
                  profile.calendarEvents.slice(0, 5).map((event) => (
                    <div key={event.id} className="family-profile-feed-card">
                      ...
                      <p className="mt-2 text-sm leading-7 text-[var(--muted)]">
                        ...
                        {event.detail || "No extra details yet."}
                      </p>
                    </div>
                  ))
                ) : (
        ...
        <div className="space-y-5">
          ...
          <DisclosurePanel
            kicker="Today's weather"
            title={displayedWeather?.weather.summary ?? "Pick a weather spot"}
            summary="Save one location and open this when you want a quick weather check."
            badge={weatherLoading ? "Loading" : displayedWeather ? "Ready" : "No spot"}
            className="family-panel rounded-[28px] p-5 md:p-6"
          >
            <p className="text-sm leading-7 text-[var(--muted)]">{displayedWeather?.location.label ?? "Add a city so the page can show today's weather."}</p>
            <form
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <div className="family-sidebar-note">
                <p className="family-kicker family-eyebrow">Temp</p>
                <p className="mt-3 font-serif text-3xl">{displayedWeather?.weather.temperatureF != null ? `${Math.round(displayedWeather.weather.temperatureF)}\u00B0` : "--"}</p>
              </div>
              <div className="family-sidebar-note">
                <p className="family-kicker family-eyebrow">Feels like</p>
                <p className="mt-3 font-serif text-3xl">{displayedWeather?.weather.feelsLikeF != null ? `${Math.round(displayedWeather.weather.feelsLikeF)}\u00B0` : "--"}</p>
              </div>
              <div className="family-sidebar-note">
                <p className="family-kicker family-eyebrow">High / low</p>
                <p className="mt-3 font-serif text-3xl">
                  {displayedWeather?.weather.highF != null && displayedWeather?.weather.lowF != null
                    ? `${Math.round(displayedWeather.weather.highF)}\u00B0 / ${Math.round(displayedWeather.weather.lowF)}\u00B0`
                    : "--"}
                </p>
              </div>
              <div className="family-sidebar-note">
                <p className="family-kicker family-eyebrow">Status</p>
                <p className="mt-3 text-sm leading-7 text-[var(--muted)]">
                  {weatherLoading
                    ? "Checking weather..."
                    : displayedWeatherError ?? (displayedWeather?.weather.windMph != null ? `${Math.round(displayedWeather.weather.windMph)} mph wind` : "Weather is ready when a location is saved.")}
                </p>
              </div>
            </div>
          </DisclosurePanel>
          ...
          <DisclosurePanel
            kicker="Fitness tracker"
            title="Track today's progress."
            summary="Open the tracker when you want to log movement, water, sleep, or share a win."
            badge={todayFitness ? "Saved today" : "Track today"}
            className="family-panel rounded-[28px] p-5 md:p-6"
          >
            <form className="family-profile-form-grid" onSubmit={saveFitness}>
              ...
              <div className="family-profile-form-grid__wide flex flex-wrap gap-3">
                ...
                {todayFitness ? (
                  <button type="button" onClick={onShareFitness} disabled={!canEdit} className="family-btn family-btn-secondary">
                    Share fitness win
                  </button>
                ) : null}
              </div>
          ...
          <DisclosurePanel
            kicker="Rewards"
            title="Trade points for rewards."
            summary="Keep reward setup tucked away until you want to add or redeem something."
          ...
          >
            ...
            <div className="mt-5 grid gap-3">
              {profile.rewards.length > 0 ? (
                ...
                <div className="family-empty rounded-[24px] p-5 text-sm leading-7 text-[var(--muted)]">Add the first reward so points feel fun instead of abstract.</div>
              )}
            </div>
          </DisclosurePanel>

          <DisclosurePanel
            kicker="Pictures and keepsakes"
            title="Save photos and favorite items."
            summary="Open this section when you want to upload a profile picture, keepsake, or favorite file."
            badge={`${profile.uploads.length} items`}
            className="family-panel rounded-[28px] p-5 md:p-6"
          >
            {canEdit ? (
              ...
              {profile.uploads.length > 0 ? (
                profile.uploads.map((upload) => (
                  <div key={upload.id} className="family-profile-upload-card">
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      <a href={upload.dataUrl} download={upload.fileName} className="family-btn family-btn-soft">
                        Open item
                      </a>
                      {canEdit ? (
                        <button type="button" onClick={() => onRemoveUpload(upload.id)} className="family-btn family-btn-secondary">
                          Remove
                        </button>
                      ) : null}
                    </div>
          ...
          <DisclosurePanel
            kicker="Shared with family"
            title="Wins everyone can celebrate."
            summary="Open this feed when you want to revisit the goals and accomplishments already shared."
            badge={`${familyFeed.length} shared`}
            className="family-panel rounded-[28px] p-5 md:p-6"
          >
            <div className="space-y-3">
              {familyFeed.length > 0 ? (
